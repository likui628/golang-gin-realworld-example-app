...
func (service *ArticleService) GetArticlesFeed(userId uint, limit, offset int) ([]ArticleOutput, error) {
	articles, err := service.repository.GetArticlesFeed(userId, limit, offset)
	if err != nil {
		return nil, err
	}
	articleIDs := make([]uint, len(articles))
